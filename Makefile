...
integration-test-full: ## Run the integration test binary with ignored tests included (requires note transport service)
	# TODO: remove -j 4 once https://github.com/0xMiden/miden-node/issues/1856 is resolved
	TEST_MIDEN_NOTE_TRANSPORT_ENDPOINT=$(NOTE_TRANSPORT_ENDPOINT) TEST_WITH_NOTE_TRANSPORT=1 cargo nextest run --workspace $(EXCLUDE_WASM_PACKAGES) --exclude testing-remote-prover --release --test=integration -j 4
	cargo nextest run --workspace $(EXCLUDE_WASM_PACKAGES) --exclude testing-remote-prover --release --test=integration --run-ignored ignored-only -j 4 -- import_genesis_accounts_can_be_used_for_transactions
