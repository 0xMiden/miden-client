...
More information on the configuration file can be found in the [configuration section](https://github.com/0xMiden/rust-sdk/docs/typedoc/rust-client/cli-config.md).
...
For `send` and `consume-notes`, you can omit the `--sender` and `--account` flags to use the default account defined in the [config](https://github.com/0xMiden/rust-sdk/docs/typedoc/rust-client/cli-config.md). If you omit the flag but have no default account defined in the config, you'll get an error instead.
