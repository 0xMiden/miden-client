...
* [BREAKING] WebClient `AccountComponent.createAuthComponentFromCommitment` now takes `AuthScheme` (enum) instead of a numeric scheme id, and `createAuthComponentFromSecretKey` is the only secret-key constructor (the old `createAuthComponent` binding is no longer exported to TS) ([#1578](https://github.com/0xMiden/miden-client/issues/1578)).
