...
* [BREAKING][rust] `BatchBuilder` now supports transactions from multiple local accounts in a single batch. `Client::new_transaction_batch` no longer takes an `AccountId`, each `BatchBuilder::push` now takes an `AccountId` argument. ([#2160](https://github.com/0xMiden/miden-client/issues/2160))
