...
* [FEATURE][web] WebClient now calls `syncState()` before `newWallet()`, `newFaucet()`, and `newAccount()` to advance the sync cursor to the chain tip, preventing a slow full-chain scan on the next sync. Controlled by the `syncBeforeNewAccount` property (default: `true`) (#1704).
