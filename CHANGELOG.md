...
* [BREAKING][param][rust] `build_wallet_id` dropped its trailing `is_mutable: bool` parameter, since code mutability is not encoded in the account ID. (#TBD)
* [BREAKING][cli] `miden-client new-account` / `new-wallet`: `--account-type <ACCOUNT_TYPE>` (short `-t`) now accepts only `private` / `public`, matching the protocol's `AccountType`, which only encodes visibility. The legacy values (`fungible-faucet`, `non-fungible-faucet`, `regular-account-immutable-code`, `regular-account-updatable-code`) are gone: code mutability is not encoded in the account ID, and non-fungible-faucet has no protocol-level kind. Faucet-vs-regular is now derived from the packages: if any package contributes the `FungibleFaucet` component, the resulting account is treated as a fungible faucet and an implicit `TokenPolicyManager` is installed when not already provided. The `new-wallet --mutable` flag was also removed, since code mutability is not encoded in the account ID. (#TBD)
