...
* [BREAKING][param][rust,store] `InputNoteRecord::new` takes a `NoteAttachments` argument; input notes persist attachments (new `attachments` column on `input_notes`). (#TBD)
* [BREAKING][param][rust] `build_wallet_id` dropped its trailing `is_mutable: bool` (code mutability isn't encoded in the account ID). (#TBD)
* [BREAKING][behavior][cli] `new-account`/`new-wallet` `--account-type` (`-t`) now accepts only `private`/`public`; legacy faucet/mutability values and `--mutable` are removed. Faucet-vs-regular is derived from packages — a `FungibleFaucet` component yields a fungible faucet with an implicit `TokenPolicyManager`. (#TBD)
* [BREAKING][type][rust] `Client::import_notes` and `sync_note_transport` return `Vec<NoteDetailsCommitment>` (was `Vec<NoteId>`); `SyncSummary::new_private_notes` too. Metadata-less imports have no `NoteId` yet — resolve via `Client::get_input_note_by_commitment`. (#TBD)
