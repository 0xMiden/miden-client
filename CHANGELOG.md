...
* [FEATURE][rust,cli] Opt-in password-based encryption for `FilesystemKeyStore`. Secret keys can now be encrypted at rest using Argon2id + ChaCha20-Poly1305 via the new `KeyEncryptor` trait and `PasswordEncryptor` implementation. Enable in the CLI by setting the `MIDEN_KEYSTORE_PASSWORD` environment variable; use programmatically via `FilesystemKeyStore::with_encryption()` or `ClientBuilder::filesystem_keystore_encrypted()`. Existing plaintext key files are auto-detected and remain readable. ([#1835](https://github.com/0xMiden/miden-client/pull/1835))
