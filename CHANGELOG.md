...
* [FEATURE][web] Coordinate concurrent `WebClient` calls with a readers-writer lock: reads run in parallel, writes take the slot exclusively. Prevents `"recursive use of an object detected"` panics from concurrent WASM access. Also expose `waitForIdle()` on `MidenClient` to drain in-flight work ([#2057](https://github.com/0xMiden/miden-client/pull/2057), [#2080](https://github.com/0xMiden/miden-client/pull/2080)).
