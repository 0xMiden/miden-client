...
* [BREAKING][removal][cli] `miden-client new-account --account-type <ACCOUNT_TYPE>` removed (along with the internal `CliAccountType` enum). The protocol's `AccountType` only encodes visibility (private/public), and the CLI's legacy values (`fungible-faucet`, `non-fungible-faucet`, `regular-account-immutable-code`, `regular-account-updatable-code`) no longer correspond to distinct protocol concepts (code mutability is not encoded in the account ID; non-fungible-faucet has no protocol-level kind). Faucet-vs-regular is now derived from the packages: if any package contributes the `FungibleFaucet` component, the resulting account is treated as a fungible faucet and an implicit `TokenPolicyManager` is installed when not already provided. (#TBD)
* [BREAKING][rename][cli] `miden-client new-account` / `new-wallet`: the short form of `--storage-mode` is now `-t` (was `-v`, which was an unintended auto-derivation from the field rename `storage_mode` → `visibility`). (#TBD)
