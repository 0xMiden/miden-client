...
* [BREAKING][type][rust] `Client::import_notes` returns `Vec<NoteDetailsCommitment>` (was `Vec<NoteId>`), since metadata-less imports have no `NoteId` yet — resolve via `Client::get_input_note_by_commitment` ([#2203](https://github.com/0xMiden/miden-client/pull/2203), [#2235](https://github.com/0xMiden/miden-client/pull/2235)).
...
* [FEATURE][rust,store] Added PSWAP chain tracking. A new `pswap_lineages` SQLite table (indexed on `state` and `current_tip_note_id`) persists one row per local-creator PSWAP order; `Client::sync_chain` walks each lineage across foreign fills via a `PswapChainObserver` registered through the new `NoteObserver` trait on `StateSync` (independent of `OnNoteReceived`; observer failures are logged but never abort sync), plus a post-sync `discover_pswap_rounds` correlator. New `Client` API: `pswap_lineages`, `pswap_lineages_for`, `pswap_lineage`, `build_pswap_cancel_by_order`. New `Store` trait methods: `upsert_pswap_lineage`, `get_pswap_lineage`, `list_pswap_lineages`, `apply_pswap_round`. A new `NoteTagSource::Subscription(NoteId)` variant (on-disk discriminant `3`) carries the asset-pair tag added at lineage creation and removed atomically when the lineage transitions to `FullyFilled` or `Reclaimed`. Reconstructed paybacks land in `input_notes` (`Unverified` state, with the captured inclusion proof) so the creator's normal consume flow finds them ([#TBD](https://github.com/0xMiden/miden-client/pull/TBD)).
