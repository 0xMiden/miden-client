...
* [BREAKING][api][rust,web] Split the sync primitives: `Client::sync_state()` now runs NTL fetch followed by on-chain sync (failing fast on either) — same combined behavior as before but with fail-fast error semantics. The on-chain-only path is exposed as `Client::sync_chain()`, and NTL-only is `Client::sync_note_transport()`. JS equivalents: `MidenClient.sync()`, `syncChain()`, `syncNoteTransport()`. ([#1930](https://github.com/0xMiden/miden-client/issues/1930))
* [BREAKING][api][web] Removed the `timeout` option from `MidenClient.sync()` / `syncChain()` / `syncNoteTransport()` and the `syncStateWithTimeout()` method from the low-level `WasmWebClient`. The parameter only rejected the caller's promise while the sync continued in the background, which was misleading. Callers who want a bail-out can wrap with `Promise.race` and accept that the sync may still complete. ([#1930](https://github.com/0xMiden/miden-client/issues/1930))
