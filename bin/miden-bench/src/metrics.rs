...
/// Runs a benchmark that returns a value (captures output size from first iteration)
pub async fn run_benchmark_with_output<F, Fut, T>(
    name: &str,
    iterations: usize,
    mut f: F,
    ...
    Fut: std::future::Future<Output = T>,
{
    ...
    for _ in 0..iterations {
        let (output, duration) = measure_time_async(&mut f).await;
        result.add_iteration(duration);

        // Capture size from first measured iteration
        if output_size.is_none() {
            output_size = Some(size_fn(&output));
        }
    }

    if let Some(size) = output_size {
        result = result.with_output_size(size);
    }
