...
pub async fn test_pass_through(client_config: ClientConfig) -> Result<()> {
    ...
    let (pass_through_note, pass_through_note_details) =
        create_pass_through_note(sender.id(), target.id(), asset.into(), client.rng())?;

    let tx_request = TransactionRequestBuilder::new()
        .own_output_notes(vec![OutputNote::Full(pass_through_note.clone())])
        .build()?;
    ...
    client.import_note(NoteFile::NoteId(pass_through_note.id())).await?;
    client.sync_state().await?;
    let input_note_record = client.get_input_note(pass_through_note.id()).await?.unwrap();
    assert!(matches!(input_note_record.state(), InputNoteState::Committed { .. }));

    let tx_request = TransactionRequestBuilder::new()
        .expected_output_recipients(vec![pass_through_note_details.recipient().clone()])
        .build_consume_notes(vec![pass_through_note.id()])
        .unwrap();
    let tx_id = execute_tx(&mut client, pass_through_account.id(), tx_request.clone()).await;
    wait_for_tx(&mut client, tx_id).await?;
...
fn create_pass_through_note(
    ...
    let pass_through_note_details = NoteDetails::new(NoteAssets::new(vec![asset])?, target_recipient);
