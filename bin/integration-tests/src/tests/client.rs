...
/// Tests that pruning old account states works correctly.
///
/// This test:
/// 1. Creates a wallet and faucet
/// 2. Mints tokens (creates faucet state change)
/// 3. Consumes the minted note (creates wallet state change)
/// 4. Sends tokens to another wallet (creates another state change for both wallets)
/// 5. Calls prune_account_history()
/// 6. Verifies that old states were pruned (one per account that had multiple states)
pub async fn test_prune_account_history(client_config: ClientConfig) -> Result<()> {
    let (mut client, authenticator) = client_config.into_client().await?;

    // Setup two wallets and a faucet
    let (first_wallet, second_wallet, faucet_account) = setup_two_wallets_and_faucet(
        &mut client,
        AccountStorageMode::Private,
        &authenticator,
        RPO_FALCON_SCHEME_ID,
    ...
    let first_wallet_id = first_wallet.id();
    let second_wallet_id = second_wallet.id();
    let faucet_id = faucet_account.id();

    // Step 1: Mint tokens to first wallet (creates faucet nonce 1 -> 2)
    let (tx_id, note) = mint_note(&mut client, first_wallet_id, faucet_id, NoteType::Private).await;
    wait_for_tx(&mut client, tx_id).await?;

    // Step 2: Consume the minted note (creates first wallet nonce 0 -> 1)
    let tx_id = consume_notes(&mut client, first_wallet_id, &[note]).await;
    wait_for_tx(&mut client, tx_id).await?;

    // Step 3: Send tokens from first wallet to second wallet
    // This creates first wallet nonce 1 -> 2
    ...
    let send_note = tx_request.expected_output_own_notes().pop().unwrap();
    execute_tx_and_sync(&mut client, first_wallet_id, tx_request).await?;

    // Step 4: Consume the sent note with second wallet (creates second wallet nonce 0 -> 1)
    let tx_id = consume_notes(&mut client, second_wallet_id, &[send_note]).await;
    wait_for_tx(&mut client, tx_id).await?;

    // At this point we should have:
    // - Faucet: 2 states (nonce 0 from creation, nonce 1 from mint)
    // - First wallet: 3 states (nonce 0 from creation, nonce 1 from consume, nonce 2 from send)
    // - Second wallet: 2 states (nonce 0 from creation, nonce 1 from consume)
    // Total old states that can be pruned: 1 (faucet) + 2 (first wallet) + 1 (second wallet) = 4

    // Prune old account states
    let pruned_count = client.prune_account_history().await?;

    // We should have pruned 4 old states (keeping only the latest state for each account)
    assert_eq!(
        pruned_count, 4,
        "Expected to prune 4 old account states, but pruned {pruned_count}"
    );

    // Verify accounts are still functional by checking their current state
    let first_wallet_record = client
        .get_account(first_wallet_id)
        .await?
        .expect("First wallet should still exist after pruning");
    let second_wallet_record = client
        .get_account(second_wallet_id)
        .await?
        .expect("Second wallet should still exist after pruning");
    let faucet_record = client
        .get_account(faucet_id)
        .await?
        .expect("Faucet should still exist after pruning");

    // Verify nonces are correct (latest state was preserved)
    // Faucet: nonce 0 (creation) -> nonce 1 (mint)
    // First wallet: nonce 0 (creation) -> nonce 1 (consume) -> nonce 2 (send)
    // Second wallet: nonce 0 (creation) -> nonce 1 (consume)
    assert_eq!(first_wallet_record.nonce().as_int(), 2, "First wallet should have nonce 2");
    assert_eq!(second_wallet_record.nonce().as_int(), 1, "Second wallet should have nonce 1");
    assert_eq!(faucet_record.nonce().as_int(), 1, "Faucet should have nonce 1");

    // Pruning again should return 0 since there are no old states left
    let second_prune_count = client.prune_account_history().await?;
    assert_eq!(second_prune_count, 0, "Second prune should not remove any states");
