...
import "@agglayer/lib/DepositContractV2.sol";
import "@agglayer/lib/GlobalExitRootLib.sol";
import "../src/DepositContractTestHelpers.sol";
...
contract ClaimAssetTestVectorsLocalTx is Test, DepositContractV2, DepositContractTestHelpers {
     ...
     *         The base contract's verifyMerkleProof is internal, so we expose it here
     *         with a different name to enable `this.verifyProof()` calls with calldata conversion.
     */
    function verifyProof(
        ...
        return verifyMerkleProof(leafHash, smtProof, index, root);
    }

    /**
     * @notice Resolves the destination address from the DESTINATION_ADDRESS environment variable.
     *         Falls back to a default address if the env var is not set.
     * @return The destination address to use in the test vectors
     */
    function _resolveDestinationAddress() internal view returns (address) {
        // Default destination address (used when env var is not set)
        address defaultAddress = 0x00000000AA0000000000bb000000cc000000Dd00;

        // Try to read DESTINATION_ADDRESS from environment
        try vm.envAddress("DESTINATION_ADDRESS") returns (address envAddr) {
            console.log("Using destination address from DESTINATION_ADDRESS env var:");
            console.log(envAddr);
            return envAddr;
        } catch {
            console.log("DESTINATION_ADDRESS env var not set, using default:");
    ...
    function test_generateClaimAssetVectorsLocalTx() public {
        ...
        require(
            this.verifyProof(leafValue, smtProofLocal, uint32(leafIndex), mainnetExitRoot),
