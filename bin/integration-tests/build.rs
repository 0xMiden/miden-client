...
//! Test functions are discovered by looking for public async functions.
...
//
// This module is automatically generated by the build script from public async test functions.
...
/// 2. Discovers public async test functions
...
/// test case information from public async functions.
///
/// # Returns
///
...
/// and identifies public async functions.
///
/// # Arguments
///
...
fn collect_test_cases_from_file(file_path: &Path) -> Vec<TestCaseInfo> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if let Some(function_name) = parse_test_function_name(trimmed) {
            // Check the next line to see if it contains ClientConfig parameter
            // This helps filter out helper functions and only include test functions
            let mut has_client_config = false;
            if i + 1 < lines.len() {
                let next_line = lines[i + 1].trim();
                has_client_config = next_line.contains("client_config") || next_line.contains("ClientConfig");
            }
            
            // Only add functions that have ClientConfig parameter
...
///
/// Looks for public async function definitions.
/// Only public functions will be added to the list of integration tests.
...
fn parse_test_function_name(line: &str) -> Option<String> {
    ...
    // Look for public async function patterns only
    let fn_pos = if tokens.get(0) == Some(&"pub") && tokens.get(1) == Some(&"async") && tokens.get(2) == Some(&"fn") {
        2 // pub async fn 
    } else {
        return None;
    };
    ...
    // Ensure there's actually a function name
    if ident.is_empty() {
        return None;
    }
...
fn generate_integration_tests(test_cases: &[TestCaseInfo]) -> String {
    ...
    for test_case in test_cases {
        // Add test_ prefix to the generated wrapper name
