...
async fn execute_transaction(
    ...
    let executed_tx = client.new_transaction(account_id, transaction_request).await?;

    // Show delta and ask for confirmation
    print_transaction_details(&executed_tx)?;
    if !force {
        println!(
    ...
    let output_notes = executed_tx.output_notes().iter().map(OutputNote::id).collect::<Vec<_>>();
    ...
    if delegated_proving {
        ...
        client.submit_transaction_with_prover(executed_tx, remote_prover).await?;
    } else {
