...
use miden_client_tests::common::execute_tx_and_sync;
use miden_lib::transaction::TransactionKernel;
use miden_objects::{
    Digest,
    account::{AccountComponent, AccountId, AccountStorageMode},
...
async fn test_counter_contract_ntx() {
    ...
    let network_note = NoteBuilder::new(native_account.id(), client.rng())
        .code(
    ...
    let tx_request = TransactionRequestBuilder::new()
        .with_own_output_notes(vec![OutputNote::Full(network_note)])
        .build()
        .unwrap();
    ...
    assert_eq!(a.storage().get_item(0).unwrap(), Digest::from([ZERO, ZERO, ZERO, Felt::new(2)]));
}
