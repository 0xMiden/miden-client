...
impl ScriptBuilder {
    ...
    pub fn link_module(&mut self, module_path: &str, module_code: &str) {
        ...
        // FIXME: Remove unwrap
        self.assembler = self.assembler.compile_and_statically_link(module_code).unwrap();
    }
    ...
    pub fn compile_tx_script(&self, tx_script: &str) -> Result<TransactionScript, JsValue> {
        ...
        let tx_script = builder.compile_tx_script(tx_script);
        Ok(tx_script)
    }
    ...
    pub fn compile_note_script(&self, program: &str) -> Result<TransactionScript, JsValue> {
        // FIXME: Do we want to consume the builder or not?
        let builder = self.clone();
        let tx_script = builder.compile_note_script(tx_script);
        Ok(tx_script)
    }

    #[wasm_bindgen(js_name = "buildLibrary")]
    pub fn build_library(&mut self, modules: Vec<&str>) -> Result<Library, JsValue> {
        // FIXME: Remove unwrap
        let library = self.assembler.assemble_library(modules).unwrap();
        Ok(library)
    }
}
