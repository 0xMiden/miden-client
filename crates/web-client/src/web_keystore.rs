...
impl<R: Rng> WebKeyStore<R> {
    ...
    pub fn new_with_callbacks(
        ...
        WebKeyStore {
            rng: Arc::new(RwLock::new(rng)),
            callbacks: Some(Arc::new(JsCallbacks {
    ...
    pub async fn add_key(&self, key: &AuthSecretKey) -> Result<(), KeyStoreError> {
        if let Some(callbacks) = &self.callbacks {
            if let Some(insert_key_cb) = &callbacks.insert_key {
    ...
    pub async fn get_key(
        ...
        if let Some(callbacks) = &self.callbacks {
            if let Some(get_key_cb) = &callbacks.get_key {
...
impl<R: Rng> TransactionAuthenticator for WebKeyStore<R> {
    ...
    async fn get_signature(
        ...
        if let Some(callbacks) = &self.callbacks {
            if let Some(sign_cb) = &callbacks.sign {
