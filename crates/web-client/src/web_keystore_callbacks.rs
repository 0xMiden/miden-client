...
use miden_client::auth::{AuthSecretKey, SigningInputs};
...
impl SignCallback {
    pub(crate) async fn sign(
        ...
        let commitment_array = Word::from(signing_inputs.to_commitment()).serialize();
        let pub_key_array = PublicKey::from(pub_key).serialize();

        let call_result = self
            .0
            .call2(&JsValue::NULL, &JsValue::from(pub_key_array), &JsValue::from(commitment_array))
