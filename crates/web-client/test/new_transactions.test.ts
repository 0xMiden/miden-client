...
test.describe("mint transaction tests", () => {
  test("multiple mints and consumes complete successfully", async ({
    client,
    sdk,
  }) => {
    ...
    const results = [];
    for (let i = 0; i < 3; i++) {
      results.push(await mockMint(client, sdk, wallet.id(), faucet.id()));
    }

    expect(results.length).toEqual(3);
    expect(results.every((r) => r.numOutputNotesCreated === 1)).toBe(true);
...
test.describe("consume transaction tests", () => {
  test("consume transaction completes successfully", async ({
    client,
    sdk,
  }) => {
    const { wallet, faucet } = await setupWalletAndFaucet(client, sdk);
    const { targetAccountBalance } = await mockMintAndConsume(
      client,
      sdk,
      wallet.id(),
      faucet.id()
    ...
    expect(targetAccountBalance).toEqual("1000");
  });
});
...
test.describe("send transaction tests", () => {
  test("send transaction completes successfully", async ({ client, sdk }) => {
    const { wallet: sender, faucet } = await setupWalletAndFaucet(client, sdk);
    ...
    const { sendCreatedNoteIds } = await mockSend(
      client,
      sdk,
      sender.id(),
      target.id(),
    ...
    await mockConsume(client, sdk, target.id(), sendCreatedNoteIds[0]);
    ...
    const senderBalance = senderAccount
      ...
      .toString();
    expect(senderBalance).toEqual("900");
  ...
  test("sends a public P2ID note, then receiver consumes it using the note's id", async ({
    ...
    // Fund sender
    await mockMintAndConsume(client, sdk, sender.id(), faucet.id());
    ...
    const balance = receiverAccount.vault().getBalance(faucet.id()).toString();
...
test.describe("custom transaction tests", () => {
  test("custom transaction completes successfully", async ({ client, sdk }) => {
  ...
  test("custom transaction fails with invalid assert", async ({
    client,
    sdk,
  }) => {
    const wallet = await client.newWallet(
    ...
    await expect(
      client.submitNewTransaction(wallet.id(), transactionRequest2)
    ).rejects.toThrow();
  });
});
...
test.describe("custom transaction with multiple output notes", () => {
  ...
  test("fails when output note serial numbers are the same", async ({
    ...
    await expect(
      client.submitNewTransaction(sender.id(), transactionRequest)
    ).rejects.toThrow();
  });
});
...
test.describe("custom account component tests", () => {
  [
    ...
    test(`custom account component transaction completes successfully (${authScheme})`, async ({
      ...
      const secretKey = sdk.AuthSecretKey[schemeSecretKeyFunction](walletSeed);
      const authComponent =
      ...
      expect(retrieveMapKey?.toHex()).toEqual(expected.toHex());
    });
  });
});
...
test.describe("storage map test", () => {
  test("storage map is updated correctly in transaction", async ({
    client,
    sdk,
  }) => {
    ...
    expect(normalizeHexWord(initialMapValue)).toBe("1");
    expect(normalizeHexWord(finalMapValue)).toBe("2");

    // Verify getMapEntries
    expect(mapEntries?.length).toBeGreaterThan(1);

    const expectedKey = MAP_KEY.toHex();
    const expectedValue = normalizeHexWord(finalMapValue);
    const hasExpectedEntry = mapEntries?.some(
      (entry) =>
        entry.key === expectedKey &&
        normalizeHexWord(entry.value) === expectedValue
    );
    expect(hasExpectedEntry).toBe(true);
  });
});
...
test.describe("submitNewTransactionWithProver tests", () => {
  ...
  test.describe("executeForSummary tests", () => {
    test("executeForSummary returns TransactionSummary for unauthorized transaction", async ({
      ...
      expect(summary.inputNotes().numNotes()).toBe(1);
      expect(summary.outputNotes().numNotes()).toBe(0);
      expect(
        summary
          .inputNotes()
          .notes()
    ...
    test("executeForSummary returns TransactionSummary for authorized transaction with matching salt", async ({
      ...
      expect(summary.inputNotes().numNotes()).toBe(0);
      expect(summary.outputNotes().numNotes()).toBe(0);
      expect(summary.salt().toHex()).toBe(expectedSalt.toHex());
    });
  });
});
