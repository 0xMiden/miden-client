...
export class WebClient {
  constructor(rpcUrl, seed, serializedMockChain) {
    console.log("WebClient: Initializing...");
    this.rpcUrl = rpcUrl;
    this.seed = seed;
  ...
  async submitTransaction(transactionResult, prover = undefined) {
    try {
      ...
      if (this.wasmWebClient.usesMockChain()) {
        ...
        console.log("Updating mock chain in index...", serializedMockChain);
  ...
  async syncState() {
    try {
      ...
      console.log("Syncing state in index...");
      if (this.wasmWebClient.usesMockChain()) {
        serializedMockChain = this.wasmWebClient.serializeMockChain().buffer;
        console.log(
          "Syncing state in index with mock chain...",
          serializedMockChain
        );
      }
