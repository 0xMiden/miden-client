...
impl StateSync {
    ...
    async fn sync_state_step(
        ...
        // 1. sync_notes drives the loop (same semantic as old sync_state)
        let note_sync =
        ...
        // 2. MMR delta for the same range
        let mmr_delta = self
            .rpc_api
        ...
        // 3. Transactions for tracked accounts (skip if none)
