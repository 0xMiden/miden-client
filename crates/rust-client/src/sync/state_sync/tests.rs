...
// PUBLIC ACCOUNT SYNC TESTS
// --------------------------------------------------------------------------------------------

#[tokio::test]
async fn sync_public_accounts_skips_fetched_state_older_than_local() {
    let mut builder = MockChainBuilder::new();
    let chain_account = builder
        .add_existing_basic_faucet(miden_testing::Auth::IncrNonce, "TST", 1_000_000, None)
        .unwrap();
    let chain_header = AccountHeader::from(&chain_account);
    ...
    let local_header = AccountHeader::new(
        chain_header.id(),
        Felt::new(chain_header.nonce().as_canonical_u64() + 1)
            .expect("test nonce should fit in the base field"),
        chain_header.code_commitment(),
        chain_header.storage_commitment(),
        chain_header.vault_root(),
    );
    let commitment_updates = BTreeMap::from([(chain_header.id(), chain_header.to_commitment())]);
    let in_flight_account_states = BTreeMap::new();
    ...
    assert!(
        ...
        "older on-chain state should be ignored while a local transaction is in flight",
    );
}
...
mod compute_nullifiers_tests {
    ...
    fn chains_rpc_transactions_by_state_commitment() {
        ...
        let records = [tx_c, tx_a, tx_b];
        let result =
            super::super::AccountTransactionsByBlock::from(records.as_slice()).ordered_nullifiers();
    ...
    fn groups_independently_by_account_and_block() {
        ...
        let records = [tx_a2, tx_b1, tx_a3, tx_a1];
        let result =
            super::super::AccountTransactionsByBlock::from(records.as_slice()).ordered_nullifiers();
    ...
    fn empty_input_returns_empty_vec() {
        let records: [RpcTransactionRecord; 0] = [];
        let result =
            super::super::AccountTransactionsByBlock::from(records.as_slice()).ordered_nullifiers();
...
fn derive_account_commitments_walks_chains_per_account() {
    let fee = FungibleAsset::new(ACCOUNT_ID_NATIVE_ASSET_FAUCET.try_into().expect("valid"), 0u64)
        .unwrap();
    ...
    let records = [tx_a_b6_1, tx_b_b6, tx_a_b5_2, tx_a_b6_2, tx_a_b5_1];
    let result = super::AccountTransactionsByBlock::from(records.as_slice()).get_account_commitments();

    assert_eq!(result.len(), 2, "one entry per account");
    assert_eq!(
        result.get(&account_a),
        Some(&word(5)),
        "account A: must walk block 6's chain, not return block 5 or an intermediate",
    );
    assert_eq!(
        result.get(&account_b),
        Some(&word(20)),
...
async fn sync_state_sets_consumed_tx_order_for_chained_transactions() {
    ...
    let state_sync = StateSync::new(Arc::new(mock_rpc.clone()), Arc::new(CommitAllScreener), None);

    let genesis_peaks = mock_rpc.get_mmr().peaks_at(Forest::new(1).expect("valid forest")).unwrap();
...
async fn erased_notes_are_marked_as_consumed() {
    ...
    let script = CodeBuilder::new()
        ...
        .unwrap();
    let recipient =
        NoteRecipient::new(Word::from([1u32, 2, 3, 4]), script, NoteStorage::new(vec![]).unwrap());
...
async fn erased_notes_are_marked_as_consumed_by_network_account() {
    ...
    let faucet_id: AccountId = ACCOUNT_ID_PRIVATE_FUNGIBLE_FAUCET.try_into().unwrap();
    let sender_account = builder.add_existing_mock_account(miden_testing::Auth::IncrNonce).unwrap();
    ...
    let script = CodeBuilder::new()
        ...
        .unwrap();
    let recipient =
        NoteRecipient::new(Word::from([7u32, 8, 9, 10]), script, NoteStorage::new(vec![]).unwrap());
...
async fn validate_chain_mmr_response_rejects_tampered_responses() {
    ...
    let header_of = |block_num: u32| mock_rpc.mock_chain.read().block_header(block_num as usize);
    let chain_mmr_response =
        || async { mock_rpc.sync_chain_mmr(current, SyncTarget::CommittedChainTip).await.unwrap() };
...
fn validate_note_blocks_range_rejects_out_of_range_blocks() {
    ...
    let result = StateSync::validate_note_blocks_range(&[genesis_note_block], current, chain_tip);
    assert!(matches!(result, Err(ClientError::ChainValidationError(_))));
}
...
fn advance_mmr_rejects_delta_inconsistent_with_chain_commitment() {
    ...
    let full_delta = mock_rpc
        ...
        .unwrap();
    StateSync::advance_mmr(full_delta, &chain_tip_header, &mut genesis_partial_mmr()).unwrap();
    ...
    let truncated_delta = mock_rpc
        ...
        .unwrap();
    let result =
        StateSync::advance_mmr(truncated_delta, &chain_tip_header, &mut genesis_partial_mmr());
    assert!(matches!(result, Err(ClientError::ChainValidationError(_))));
}
