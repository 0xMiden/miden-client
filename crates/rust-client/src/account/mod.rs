...
impl<AUTH> Client<AUTH> {
    ...
    /// - States referenced by pending transactions (needed for rollback if transactions are discarded)
    ///
    /// # What is pruned
    ///
    /// - Old committed account states that are no longer needed
    /// - Orphaned storage, vault, and map data no longer referenced by any account state
    ///
    /// # Returns
    ///
    /// Information about what was pruned, including the number of states and orphaned rows deleted.
    pub async fn prune_account_history(
        &self,
        account_id: AccountId,
    ) -> Result<crate::store::PrunableAccountData, ClientError> {
        self.store.prune_account_history(account_id).await.map_err(ClientError::StoreError)
    }
}
