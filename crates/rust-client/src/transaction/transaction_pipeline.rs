...
impl TransactionPipeline {
    ...
    pub async fn prove_transaction(
        ...
        let proof = prover.prove(executed.into()).await?;
        self.proven_transaction = Some(proof.clone());

        Ok(proof)
    }

    /// Submits the proven transaction previously generated by [`Self::prove_transaction`] to the
    /// network and returns the resulting [`TransactionStoreUpdate`].
    pub async fn submit_proven_transaction(
        &mut self,
    ) -> Result<TransactionStoreUpdate, ClientError> {
        if self.submission_height.is_none() {
            let proven = self
                .proven_transaction
                .clone()
