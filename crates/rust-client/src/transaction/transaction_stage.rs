...
use miden_objects::note::{NoteDetails, NoteRecipient, NoteTag};
use miden_objects::transaction::{
    AccountInputs, ExecutedTransaction, InputNote, InputNotes, TransactionArgs,
};
use miden_objects::{AssetError, Word};
use miden_tx::auth::TransactionAuthenticator;
use miden_tx::{NoteAccountExecution, NoteConsumptionChecker, TransactionExecutor};

use crate::ClientError;
use crate::rpc::NodeRpcClient;
use crate::store::data_store::ClientDataStore;
use crate::transaction::{TransactionRequest, TransactionResult, TransactionScriptTemplate};
...
impl TransactionStages {
    pub fn new(
        rpc_api: Arc<dyn NodeRpcClient + Send>,
        max_block_number_delta: Option<u32>,
    ) -> Self {
        Self { rpc_api, max_block_number_delta }
    }
    ...
    pub async fn execute_transaction(
        &mut self,
        account: Account, //TODO: Should this be reduced?
        transaction_request: TransactionRequest,
        foreign_account_inputs: Vec<AccountInputs>,
        mut input_notes: InputNotes<InputNote>,
        data_store: &ClientDataStore,
        executor: &TransactionExecutor<'_, '_, ClientDataStore, impl TransactionAuthenticator>,
        ...
        let tx_script = transaction_request.build_transaction_script(
            &(&account).into(),
            true.into(), /* TODO: Receive debug mode */
        )?;
