...
impl Client {
    ...
    pub async fn insert_new_wallet(
        ...
        self.insert_new_wallet_with_seed(storage_mode, keystore, init_seed).await
    }

    /// Builds a new wallet account with the provided seed and inserts it into the client and
    /// keystore.
    pub async fn insert_new_wallet_with_seed(
        ...
        let key_pair = SecretKey::with_rng(self.rng());
        ...
        self.add_account(&account, Some(seed), false).await?;
    ...
    ///
    /// This function creates a set of basic accounts and faucets, and mints tokens from each faucet
    /// to the accounts based on the given balance matrix.
    ///
    /// # Arguments
    ///
    ...
    /// * `balances` - A matrix where each row represents a faucet and each column a wallet. Every
    ///   entry represents the number of tokens to mint from a faucet to an wallet.
    ///
    /// # Returns
    ///
    ...
    pub async fn setup_accounts_and_faucets(
        ...
        for _ in 0..num_accounts {
            let (account, ..) = self.insert_new_wallet(storage_mode, keystore).await?;
            accounts.push(account);
        }

        let mut faucets = Vec::with_capacity(num_faucets);
        for _ in 0..num_faucets {
            let (faucet, ..) = self.insert_new_fungible_faucet(storage_mode, keystore).await?;
            faucets.push(faucet);
        }
        ...
        for (faucet_index, faucet) in faucets.iter().enumerate() {
            for (acct_index, account) in accounts.iter().enumerate() {
                ...
                self.execute_tx_and_consume_output_notes(tx_request, faucet.id(), account.id())
                    .await?;
            }
        }
