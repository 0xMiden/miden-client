...
// WIT scalar types the CLI encodes from / renders as a single hex value instead of field by
// field. We recognize them by the last `/`-segment of the debug type name, which assumes the
// compiler keeps these names (the Walnut fork emits `miden:base/core-types@1.0.0/account-id`; a
// bare `account-id` also matches). Update here if the upstream compiler renames these structs.
const ACCOUNT_ID_TYPE: &str = "account-id";
const WORD_TYPE: &str = "word";

pub(super) fn is_account_id_type(name: &str) -> bool {
    wit_type_name(name) == ACCOUNT_ID_TYPE
}

/// The compiler lowers WIT `word` (a record of 4 felts) to a named struct and never emits
/// `DebugPrimitiveType::Word`, so this name check is what fires, not the `Word` primitive arm.
pub(super) fn is_word_type(name: &str) -> bool {
    wit_type_name(name) == WORD_TYPE
}
...
mod tests {
    ...
    use miden_mast_package::debug_info::{DebugFunctionInfo, DebugFunctionsSection, DebugTypeIdx};

    use super::{find_debug_fn, proc_display_name};
