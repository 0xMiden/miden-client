...
pub struct ClientBuilder {
    ...
    rpc_api: Option<Arc<dyn NodeRpcClient + Send + Sync>>,
...
impl ClientBuilder {
    ...
    #[must_use]
    pub fn with_rpc(mut self, client: Arc<dyn NodeRpcClient + Send + Sync>) -> Self {
...
impl ClientBuilder {
    ...
    pub async fn build(mut self) -> Result<Client, ClientError> {
        ...
        let rpc_api: Arc<dyn NodeRpcClient + Send + Sync> = if let Some(client) = self.rpc_api {
            client
        } else {
