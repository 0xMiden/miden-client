...
use crate::store::sqlite_store::merkle_store::{insert_asset_paths, update_asset_node};
...
impl SqliteStore {
    ...
    pub(crate) fn insert_account(
        ...
        insert_asset_paths(&mut merkle_store, account.vault())?;
        Self::insert_assets(
            &tx,
            &mut merkle_store,
            account.vault().root(),
    ...
    pub(crate) fn get_account_asset(
    ...
    ) -> Result<Option<(Asset, Word)>, StoreError> {
        let header = Self::get_account_header(conn, account_id)?
            .ok_or(StoreError::AccountDataNotFound(account_id))?
            .0;

        let asset = query_vault_assets(
            conn,
        ...
        Ok(asset.map(|asset| (asset, header.vault_root())))
    }
    ...
    pub(super) fn update_account_state(
        ...
        insert_asset_paths(merkle_store, new_account_state.vault())?;
        Self::insert_assets(
            tx,
            merkle_store,
    ...
    fn insert_storage_slots<'a>(
        ...
        for (index, slot) in account_storage {
            const QUERY: &str =
                insert_sql!(account_storage { commitment, slot_index, slot_value } | REPLACE);
    ...
    fn insert_assets(
        ...
        for asset in assets {
            ...
            update_asset_node(merkle_store, old_root, &asset)?;
        }
...
fn query_storage_slots(
    ...
    const STORAGE_QUERY: &str = "SELECT slot_index, slot_value FROM account_storage WHERE ";
    ...
    let storage_values = conn
        ...
        .map(|result| {
            ...
            Ok((index, Word::try_from(value)?))
        })
        .collect::<Result<Vec<(u8, Word)>, StoreError>>()?;

    let possible_roots: Vec<Value> =
        storage_values.iter().map(|(_, value)| Value::from(value.to_hex())).collect();
    ...
    Ok(storage_values
        ...
        .map(|(index, value)| {
            if let Some(map) = storage_maps.remove(&value) {
                (index, StorageSlot::Map(map))
            } else {
                (index, StorageSlot::Value(value))
            }
        })
        .collect())
}
