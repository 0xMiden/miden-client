...
async fn get_output_notes() {
    ...
    let (first_regular_account, faucet_account_header) =
        setup_wallet_and_faucet(&mut client, AccountStorageMode::Private, &authenticator).await.unwrap();
...
async fn subsequent_discarded_transactions() {
    ...
    let (regular_account, faucet_account_header) =
        setup_wallet_and_faucet(&mut client, AccountStorageMode::Public, &keystore).await.unwrap();
...
async fn swap_chain_test() {
    ...
    for _ in 0..3 {
        let (wallet, faucet) =
            setup_wallet_and_faucet(&mut client, AccountStorageMode::Private, &keystore).await.unwrap();
