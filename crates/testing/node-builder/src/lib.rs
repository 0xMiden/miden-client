...
use miden_protocol::account::{Account, AccountBuilder, AccountComponent, AccountComponentMetadata, AccountFile, StorageMap};
...
fn create_test_account_with_many_assets(faucets: &[Account]) -> anyhow::Result<Account> {
    ...
    let acc_component = AccountComponent::new(basic_wallet_library(), vec![storage_map], AccountComponentMetadata::new("miden::testing::basic_wallet"))
