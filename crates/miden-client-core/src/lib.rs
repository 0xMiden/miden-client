...
pub use miden_tx::LocalTransactionProver;
    ...
    AUTH: TransactionAuthenticator,
{
    ...
    /// Returns a clone of the client's underlying store handle.
    pub fn store(&self) -> Arc<dyn Store> {
        self.store.clone()
    }

    /// Returns a clone of the local transaction prover used by the client.
    pub fn transaction_prover(&self) -> Arc<LocalTransactionProver> {
