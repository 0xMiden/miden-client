    ...
    AUTH: TransactionAuthenticator + Sync + 'static,
{
    ...
    pub async fn submit_transaction_with_prover(
        ...
        self.finalize_proven_transaction(proven_transaction, tx_result).await
    }

    /// Finalizes a previously proven transaction by submitting it to the network and applying the
    /// resulting state transition to the local store.
    pub async fn finalize_proven_transaction(
        &mut self,
