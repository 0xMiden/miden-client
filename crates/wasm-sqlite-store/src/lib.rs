//! Provides a SQLite-backed implementation of the [`Store`] trait for WASM environments.
//!
//! This module enables persistence of client data (accounts, transactions, notes, block headers,
//! etc.) using `SQLite` when running in WebAssembly. It uses wasm-bindgen to interface with
//! JavaScript `SQLite` adapters, supporting both Node.js (via better-sqlite3) and browsers
//! (via wa-sqlite + OPFS/IDB VFS).
//!
//! The database schema is identical to the native `sqlite-store` crate, ensuring full
//! compatibility of database files between native and WASM environments.
//!
...
// Initialize SQLite database via JS adapter
#[wasm_bindgen(module = "/src/js/schema.js")]
extern "C" {
    /// Opens the database and registers it in the JS registry.
    /// Returns the database ID which can be used to look up the database.
    #[wasm_bindgen(js_name = openDatabase)]
    fn open_database(db_name: &str, client_version: &str) -> js_sys::Promise;
}

#[wasm_bindgen(module = "/src/js/utils.js")]
extern "C" {
    #[wasm_bindgen(js_name = logError)]
    fn log_error(error: JsValue, error_context: alloc::string::String);
}

/// `WasmSqliteStore` provides a SQLite-backed implementation of the Store trait for WASM.
///
/// The database is accessed through a JavaScript `SQLite` adapter layer that supports
/// multiple backends (better-sqlite3 for Node.js, wa-sqlite for browsers).
...
impl WasmSqliteStore {
    pub async fn new(database_name: String) -> Result<WasmSqliteStore, JsValue> {
        let promise = open_database(database_name.as_str(), CLIENT_VERSION);
        let _db_id = wasm_bindgen_futures::JsFuture::from(promise).await?;
        Ok(WasmSqliteStore { database_id: database_name })
    }
...
impl Store for WasmSqliteStore {
    ...
    async fn get_account_code(
        ...
        Ok(Some(self.get_account_code_by_commitment(header.code_commitment()).await?))
    }
    ...
    async fn insert_address(
        ...
        self.insert_address_record(address, &account_id).await
    }
