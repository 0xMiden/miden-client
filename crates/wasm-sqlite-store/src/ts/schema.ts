...
import { createAdapter, SqliteAdapter } from "./adapter.js";
...
const databaseRegistry = new Map<string, SqliteAdapter>();

/**
 * Get a database instance from the registry by its ID.
 * Throws if the database hasn't been opened yet.
 */
export function getDatabase(dbId: string): SqliteAdapter {
...
function ensureClientVersion(
  ...
  const row = adapter.get<{ value: Uint8Array }>(
  ...
  const storedVersion =
    row.value instanceof Uint8Array
      ...
      : decoder.decode(new Uint8Array(row.value as ArrayBuffer));
