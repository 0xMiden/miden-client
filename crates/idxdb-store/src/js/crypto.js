...
async function getOrCreateEncryptionKey(dbId) {
    ...
    const stored = (await db.dexie.table("settings").get("encryptionKey"));
    let key;
...
export async function encryptSecretKey(dbId, plaintext) {
...
export async function decryptSecretKey(dbId, encrypted, iv) {
    const key = await getOrCreateEncryptionKey(dbId);
