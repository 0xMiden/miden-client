...
export async function pruneOldAccountStates(
  dbId: string,
  pendingAccountCommitments: string[]
): Promise<number> {
  try {
    const db = getDatabase(dbId);
    ...
    const accountsByIdMap: Map<string, IAccount[]> = new Map();
    await db.accounts.each((record) => {
    ...
    const vaultRootsToCheck: Set<string> = new Set();
    ...
    for (const records of accountsByIdMap.values()) {
      ...
      for (let i = 1; i < records.length; i++) {
        ...
        if (!pendingSet.has(record.accountCommitment)) {
          ...
          if (record.vaultRoot) vaultRootsToCheck.add(record.vaultRoot);
          if (record.storageRoot) storageRootsToCheck.add(record.storageRoot);
          if (record.codeRoot) codeRootsToCheck.add(record.codeRoot);
        }
      }
    }
