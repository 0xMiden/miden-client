...
function isNewerNonce(candidate: string, current: string): boolean {
  return BigInt(candidate) > BigInt(current);
}

function toHeaderObject(record: IAccount) {
...
function pickLatestRecord(records: IAccount[]): IAccount | undefined {
  if (records.length === 0) {
    return undefined;
  }

  return records.reduce((latest, record) =>
    isNewerNonce(record.nonce, latest.nonce) ? record : latest
  );
}
    ...
    await db.dexie.transaction(
      ...
      async () => {
        ...
        const affectedAccountIds = Array.from(
          new Set(removedStates.map((state) => state.id))
        );
